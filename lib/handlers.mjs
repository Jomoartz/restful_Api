...
// Required data: id
// Optional data: none
handler.__tokens.get = async (data) => {
  const ver = await valiateData(data);
  // Check that id is valid

  if (ver.id) {
    // Lookup the token
    const tokenData = await __data.read("tokens", ver.id);
    if (tokenData) {
    ...
    return [400, { Error: "Missing required field, or field invalid" }];
  }
};
...
// Optional data: none
handler.__tokens.put = async (data) => {
  const ver = await valiateData(data);
  if (ver.id && ver.extend) {
    // Lookup the existing token
    const tokenData = await __data.read("tokens", ver.id);

    if (tokenData) {
      console.log(Date.now(), " vs ", tokenData.expires);
      // Check to make sure the token isn't already expired
      if (tokenData.expires > 1000) {
        console.log(Date.now(), " vs ", tokenData.expires);
        // Set the expiration an hour from now
        tokenData.expires = Date.now() + 1000 * 60 * 60;
        // Store the new updates
        try {
          await __data.update("tokens", ver.id, tokenData);
          return [200, { newId: tokenData.expires }];
        } catch (error) {
          console.log("error updatinng data");
        }
      } else {
        return [
          400,
      ...
      return [400, { Error: "Specified user does not exist." }];
    }
  } else {
    return [
      400,
...
// Required data: id
// Optional data: none
handler.__tokens.delete = async (data) => {
  // Check that id is valid
  const ver = await valiateData(data);
  console.log(ver.id);
  if (ver.id) {
    ...
    if (tokenData) {
      // Delete the token
      try {
        await __data.delete("tokens", ver.id);
    ...
    return [400, { Error: "Missing required field" }];
  }
};
